...
public final class ProjectorItem extends BlockItem {
    public static final ItemContainerContents DEFAULT_CONTAINER_ITEMS = Util.make(() -> {
        var list = new ArrayList<Optional<ItemStackTemplate>>(1);
        var item = ModRegistries.SLIDE_ITEM.get();
        list.add(Optional.of(new ItemStackTemplate(item)));
    ...
    public ProjectorItem(Identifier identifier) {
        super(ModRegistries.PROJECTOR_BLOCK.get(), new Properties()
                .rarity(Rarity.RARE)
                .component(DataComponents.CONTAINER, DEFAULT_CONTAINER_ITEMS)
